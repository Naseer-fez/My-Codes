...
reqcoloums=['track_album_release_date','track_name','playlist_genre','track_popularity','playlist_subgenre','danceability','loudness','liveness','time_signature']
Dataset=Dataset[reqcoloums]
start_date='2000-01-01'
end_date='2024-12-31'
Dataset=Dataset[(Dataset['track_album_release_date']>=start_date) & (Dataset['track_album_release_date']<=end_date)]
...
def userdata_numeric():
    ...
    df=user_profiles
    numeric_features=df[['min_popularity','max_popularity','likes_danceable','max_loudness']].copy()
    numeric_features['likes_danceable']=numeric_features['likes_danceable'].astype(int)
    numeric_array=numeric_features.to_numpy()
    return numeric_array

reqcoloums=['track_popularity','danceability','loudness','playlist_genre','playlist_subgenre']
user_data=userdata()
UserData_array=userdata_numeric()
Spotifydata_mod=np.array((Dataset['track_popularity'],Dataset['loudness'],Dataset['danceability']))
...
class MusicRecomendationData:
    ...
    def stuff(self,user_no):
        self.user_data=self.user_genre
    ...
    def algo_ranging(self,user_no):
        self.range_varation_popularity=(((self.user_popularity[user_no])+self.varation_spotify_data[0]),((self.user_popularity[user_no])-self.varation_spotify_data[0]))
        self.range_varation_loudness=(((self.user_loudness[user_no])+self.varation_spotify_data[2]),((self.user_loudness[user_no])-self.varation_spotify_data[2]))
        self.range_varation_dancablity=(((self.user_dancable[user_no])+self.varation_spotify_data[1]),((self.user_dancable[user_no])-self.varation_spotify_data[1]))
    ...
    def perfoming_filterantion(self,user_no):
        ...
        self.Sorting_data_pop=self.Sorting_data[(self.Sorting_data['track_popularity']<=high_pop)&(self.Sorting_data['track_popularity']>=low_pop)]
        self.Sorting_data_loud=self.Sorting_data[(self.Sorting_data['loudness']>=low_loud)&(self.Sorting_data['loudness']<=high_loud)]
        self.Sorting_data_dance=self.Sorting_data[(self.Sorting_data['danceability']>=low_dance)&(self.Sorting_data['danceability']<=high_dance)]
    ...
    def data_manuplating(self,user_no):
        ...
        single_user=user_data.iloc[user_no]
        self.filtered_data=self.Sorting_data
...
class Algorithm:
    ...
    def algo_ranging(self):
        self.range_varation_popularity=(((self.md.user_popularity[self.user_no])+self.md.varation_spotify_data[0]),((self.md.user_popularity[self.user_no])-self.md.varation_spotify_data[0]))
        self.range_varation_loudness=(((self.md.user_loudness[self.user_no])+self.md.varation_spotify_data[2]),((self.md.user_loudness[self.user_no])-self.md.varation_spotify_data[2]))
        self.range_varation_dancablity=(((self.md.user_dancable[self.user_no])+self.md.varation_spotify_data[1]),((self.md.user_dancable[self.user_no])-self.md.varation_spotify_data[1]))
        self.range_all=((self.range_varation_popularity),(self.range_varation_loudness),(self.range_varation_dancablity))
        self.range_all=np.array(self.range_all)
        return self.range_all
    ...
    def data_manuplating(self,user_no):
        ...
        single_user=user_data.iloc[self.user_no]
        self.filtered_data=self.Sorting_data[(self.Sorting_data['track_popularity']<(single_user['max_popularity'])) & (self.Sorting_data['track_popularity']>(single_user['min_popularity']))]
        return self.filtered_data

    def algo_making(self,user_no):
        self.final_data=self.data_manuplating(self.user_no).copy()
        self.user_property=[self.md.user_popularity[self.user_no],self.md.user_loudness[self.user_no],self.md.user_dancable[self.user_no]]
        self.data_property=['track_popularity','loudness','danceability']
        self.user_dates=(pd.to_datetime(user_data['release_start'][self.user_no],errors='coerce'),pd.to_datetime(user_data['release_end'][self.user_no],errors='coerce'))
        self.final_data['track_album_release_date']=pd.to_datetime(self.final_data['track_album_release_date'],errors='coerce')
    ...
    def perofoming_algo(self,user_no,year):
        ...
        self.mod_data=self.final_data[(self.final_data['track_album_release_date']>=(self.user_dates[0]-spanyears)) & (self.final_data['track_album_release_date']<=(self.user_dates[1]+spanyears))]
        if year<=0:
            self.retrevingdata(self.user_no)
        ...
        match value:
            ...
            case 3:
                if(data.shape[0]>=1):
                    ...
                    returining_data=data['track_name'].iloc[0]
                    return returining_data
            case 4:
